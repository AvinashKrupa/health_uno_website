...
const AddPrescription = (props) => {
  ...
  const [investigationRequiredCheck, setInvestigationRequiredCheck] =
  ...
  function handleChooseTemplate(itemIndex) {
    setChosenTemplate(itemIndex);
  }

  function deleteTemplates() {
    let templateIdArr = [];
    templateIdArr.push(savedPrescription[chosenTemplate]._id);
    let params = {
      ids: templateIdArr,
    };
  ...
  function reducer(prescription_list, action) {
    switch (action.type) {
      ...
      case ACTIONS.APPEND_CHOSEN_TEMPLATE:
        let finalPrescriptionObj = JSON.parse(
          JSON.stringify(prescription_list)
        );
      ...
      case ACTIONS.DELETE_MEDICINE:
        const tempPrescriptionObj = JSON.parse(
          JSON.stringify(prescription_list)
        );
      ...
      case ACTIONS.CHANGE_MEDICINE_NAME:
        ...
        prescription_list[action.payload.id].medicineItem.medicineName =
      ...
      case ACTIONS.SET_DOSAGE_TYPE:
        prescription_list[action.payload.id].medicineItem.dosage.qty =
          action.payload.value;
        return [...prescription_list];
      case ACTIONS.SET_START_DATE:
        prescription_list[action.payload.id].medicineItem.start_date =
          action.payload.value;
        return [...prescription_list];
      case ACTIONS.SET_DAYS:
        prescription_list[action.payload.id].medicineItem.days =
          action.payload.value;
        return [...prescription_list];
      case ACTIONS.SET_PERIODITY:
        prescription_list[action.payload.id].medicineItem.periodicity =
          action.payload.value;
        return [...prescription_list];
      ...
      case ACTIONS.CHANGE_SOS:
        prescription_list[action.payload.id].medicineItem.dosage.sos =
          action.payload.value;
        return [...prescription_list];
      case ACTIONS.CHANGE_STAT:
        prescription_list[action.payload.id].medicineItem.dosage.stat =
          action.payload.value;
        return [...prescription_list];
      case ACTIONS.ADD_TIME_SLOT:
        if (
          !prescription_list[
            action.payload.id
          ].medicineItem.time_slots.includes(action.payload.value)
        ) {
          prescription_list[action.payload.id].medicineItem.time_slots.push(
            action.payload.value
          );
        }
        return [...prescription_list];
      case ACTIONS.REMOVE_TIME_SLOT:
        const arr = prescription_list[
          action.payload.id
        ].medicineItem.time_slots.filter((e) => e !== action.payload.value);
      ...
      case ACTIONS.CHANGE_DOSAGE_SLOT:
        switch (action.payload.value) {
          case "beforeFood":
            prescription_list[
              action.payload.id
            ].medicineItem.dosage.before_food = true;
            prescription_list[
              action.payload.id
            ].medicineItem.dosage.after_food = false;
            prescription_list[
              action.payload.id
            ].medicineItem.dosage.with_food = false;
            prescription_list[
              action.payload.id
            ].medicineItem.dosage.other = false;
            prescription_list[
              action.payload.id
            ].medicineItem.dosage.other_details = "";
            return [...prescription_list];
          case "afterFood":
            prescription_list[
              action.payload.id
            ].medicineItem.dosage.before_food = false;
            prescription_list[
              action.payload.id
            ].medicineItem.dosage.after_food = true;
            prescription_list[
              action.payload.id
            ].medicineItem.dosage.with_food = false;
            prescription_list[
          ...
          case "withFood":
            ...
            prescription_list[
              action.payload.id
            ].medicineItem.dosage.with_food = true;
            prescription_list[
              action.payload.id
            ].medicineItem.dosage.other = false;
            prescription_list[
          ...
          case "otherFood":
            ...
            prescription_list[
              action.payload.id
            ].medicineItem.dosage.other = true;
            prescription_list[
              action.payload.id
            ].medicineItem.dosage.other_details = "";
            return [...prescription_list];
      ...
      case ACTIONS.VALIDATE_MEDICINE_INFO:
        ...
        if (
          ...
          prescription_list[
            action.payload.id
          ].validationInfo.medicine_type_error =
            "Please add dosage information";
        } else {
          prescription_list[
            action.payload.id
          ].validationInfo.medicine_type_error = "";
  ...
  const handleProceed = () => {
    dispatch({
      type: ACTIONS.CHANGE_PRESCRIPTION_TYPE,
      payload: { id: selectedSectionIndex, value: tempPrescriptionType },
    });
    setOpenDialog(false);
  };
  ...
  const submitPrescription = () => {
    ...
    post(API.SUBMIT_PRESCRIPTION, params)
      ...
      .catch((error) => {
        setShowLoader(false);
        addToast(
          error.response?.data?.message?.investigations?.message ||
        ...
        >
          <DialogContent>
            <div className="modal-title-container" style={{ marginTop: 20 }}>
              <div
                className="choose-template-title"
          ...
          <div className="addPrescription">
            <Row className="topsctionName">
              ...
              <Row className={"prescription-header"}>
                ...
                <Col md className="Choosetemplate">
                  <p
                    className="chooseTemplateButton"
                    onClick={() => handleChooseTempClickOpen()}
                  >
                    Choose template
                  </p>
                </Col>
            ...
            <div className="actionSave">
              ...
              <Button
                ...
                onClick={() => {
                  dispatch({
                    type: ACTIONS.ADD_NEW_MEDICINE,
                    payload: prescriptionObj,
                  });
                  dispatch({
                        ...
                        <Col key={index} lg="6" md sm="12" xs="12">
                          ...
                          >
                            <TextArea
                              ...
                              value={text}
                              onChange={(value) =>
                                updateInvestigationState(index, value)
                              }
                              maxLength="200"
                            ></TextArea>
            ...
            <Row className="sendPrescriptionAction">
              ...
              {!prescription_list.length && (
                <span className="error-text">
                  Please add a medicine to proceed
                </span>
              )}
              {!userShouldProceed && prescription_list.length > 1 && (
                <span className="error-text">
                  Please fill/correct required fields{" "}
                </span>
              )}
            </Row>
          </div>
